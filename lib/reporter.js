...
class AllureReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        ...
        this.on('test:start', (test) => {
            ...
            currentTest.addParameter('environment-variable', 'testcases', JSON.stringify(test.testcases)) // check if works
            currentTest.addParameter('environment-variable', 'spec files', JSON.stringify(test.specs)) // check why only the first one is being displayed

            allure.workflo.failedSpecs = []
            allure.workflo.pendingSpecCriterias = []
            allure.workflo.pendingSpec = undefined
        })
        ...
        */
        ...
        this.on('step:failed', function (step) {
            ...
            this.addVerifiedSpecs(step.assertion.specObj)
        })

        this.on('step:succeeded', function (step) {
            this.addVerifiedSpecs(step.assertion.specObj)
        })
        ...
        this.on('step:end', function (step) {
            ...
            if (step.verify) {
                if (this.didSpecFail(step.verify.storyId, step.verify.criteriaId)) {
                    status = 'failed'
                ...
                } else if (!this.wasSpecVerified(step.verify.storyId, step.verify.criteriaId)) {
                    allure.workflo.pendingSpecCriterias.push(step.verify.criteriaId)
                    allure.workflo.pendingSpec = step.verify.storyId
                    status = 'failed'
                }
            }
    ...
    getVerifiedSpecs () {
        return this.verifiedSpecs
    }

    addVerifiedSpecs (specObj) {
        const verifiedSpecs = this.verifiedSpecs

        for (const key in specObj) {
            if (!(key in verifiedSpecs)) {
                verifiedSpecs[key] = {}
            }

            let criterias = specObj[key]
            ...
            for (const criteria of criterias) {
                if (!(criteria in verifiedSpecs[key])) {
                    verifiedSpecs[key][criteria] = true
                }
            }
        }
    }

    wasSpecVerified (storyId, criteriaId) {
        const verifiedSpecs = this.verifiedSpecs

        if (storyId in verifiedSpecs &&
            criteriaId in verifiedSpecs[storyId]) {
            return true
        } else {
            return false
        }
    ...
    didSpecFail (storyId, criteriaId) {
        const failedSpecs = this.failedSpecs

        if (storyId in failedSpecs &&
            criteriaId in failedSpecs[storyId]) {
            return true
        } else {
            return false
        }
