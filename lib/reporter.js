...
class AllureReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        ...
        this.on('test:setCurrentId', (test) => {
            ...
            if (this.currentTestId) {
                if (test.testcase) {
                    ...
                    currentTest.addParameter('argument', 'Verifies Specs', traceInfo.specs.join(', '))
                } else if (test.spec) {
                    ...
                    const manualFile = traceInfo.criteriaVerificationFiles[criteria].manualFile
                    const testcases = traceInfo.criteriaVerificationFiles[criteria].testcases

                    if (manualFile) {
                        currentTest.addParameter('argument', 'Verified by Manual Results', manualFile)
                    } else if (testcases.length > 0) {
                        currentTest.addParameter('argument', 'Verified in Testcases', testcases.join(', '))
                    }
                }
            }
        })
        ...
        */
        ...
        this.on('step:fail', function (step) {
            ...
            this.dumpJSON(allure, 'Verification Failures', step.errs)
        ...
        this.on('step:broken', function (step) {
            ...
            this.dumpJSON(allure, 'Errors', step.assertion)

            if (step.assertion.screenshotFilename) {
                allure.addAttachment(step.assertion.message, new Buffer(fs.readFileSync(step.assertion.screenshotFilename, {encoding: 'base64'}), 'base64'))
            }
    ...
    buildResultStr (results) {
        ...
        if (results.unverified.count > 0) {
            str += `, ${results.unverified.count} unverified (~${Math.round(results.unverified.percentage)}%)`
        }
