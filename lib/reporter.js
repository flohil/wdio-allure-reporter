    ...
    constructor (baseReporter, config, options = {}) {
        ...
        this.on('test:pass', (test) => {
            ...
            if (allure.workflo.specMode) {
                if (allure.workflo.failedSpecs.length > 0) {
                    ...
                    err = newErr
                } else if (allure.workflo.pendingSpecCriterias.length > 0) {
                    status = 'failed'
                    err = {
                        message: `Spec ${allure.workflo.pendingSpec}: ${allure.workflo.pendingSpecCriterias.toString()} was not verified!`,
                        stack: ''
                    }
        ...
        */
        ...
        this.on('step:end', function (step) {
            const allure = this.getAllure(step.cid)
            const failedSpecs = this.getFailedSpecs()
            const verifiedSpecs = this.getVerifiedSpecs()
