...
class AllureReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        ...
        this.dumpFailuresAndErrors = function (allure, _assertions) {
            ...
            _assertions.forEach(assertion => {
                const _assertion = _.cloneDeep(assertion)
            ...
            var failures = Object.values(assertions).filter(function (assertion) { return typeof assertion.matcherName !== 'undefined' })
            var errors = Object.values(assertions).filter(function (assertion) { return typeof assertion.matcherName === 'undefined' })
        ...
        this.dumpScreenshots = function (allure, _screenshots) {
            for (var screenshotMessage in _screenshots) {
                ...
                Object.values(screenshots).forEach(screenshot => {
        ...
        this.on('runner:command', (command) => {
            ...
            if (debugSeleniumCommand) {
                this.startStep(command.cid, `${command.method} ${command.uri.path}`)
        ...
        this.on('runner:result', (command) => {
            ...
            if (debugSeleniumCommand) {
                ...
                this.endStep(command.cid, status)
            }
        })
        ...
        this.on('retry:failed', function (step) {
            ...
            if (this.retryMessage.assertion.screenshotFilename) {
                allure.addAttachment(
                    'Screenshot_' + this.retryMessage.assertion.screenshotId + ': \'' + this.retryMessage.assertion.message + '\'',
                    new Buffer(fs.readFileSync(this.retryMessage.assertion.screenshotFilename, { encoding: 'base64' }), 'base64')
                )
            }
    ...
    getAllure (cid) {
        ...
        console.log('outputDir: ', this.options)
    ...
    addAllurePropertiesFile () {
        try {
            ...
            if (this.config.allure.bugTrackerPattern) {
                allurePropertiesContents.push('allure.tests.management.pattern=' + this.config.allure.bugTrackerPattern)
            }
